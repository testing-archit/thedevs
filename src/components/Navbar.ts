                ...
                <div class="flex items-center justify-between h-16">
                    ...
                    <div class="hidden md:flex items-center space-x-6">
                        ...
                        <div class="flex items-center space-x-4">
                            ...
                            <form method="post" action="/api/auth/logout" hx-post="/api/auth/logout" hx-target="body" hx-swap="outerHTML">
                                <button type="submit" class="px-4 py-2 text-gray-300 hover:text-white transition-colors border border-white/20 rounded-lg hover:border-white/40">
                                    Logout
                                </button>
                            </form>
                        </div>
            ...
            <div x-show="mobileMenuOpen" x-transition class="md:hidden glass-card border-t border-white/10">
                <div class="px-4 py-4 space-y-3">
                    ...
                    <div class="pt-3 border-t border-white/10">
                        <p class="text-gray-400 text-sm mb-2">${user.name}</p>
                        <form method="post" action="/api/auth/logout" hx-post="/api/auth/logout" hx-target="body" hx-swap="outerHTML">
                            <button type="submit" class="w-full px-4 py-2 text-gray-300 hover:text-white transition-colors border border-white/20 rounded-lg hover:border-white/40">
                                Logout
                            </button>
                        </form>
                    </div>
